...
local function resetfish()
	...
	keyrelease(0x41)
	task.wait(0.5)
	root.CFrame = CFrame.new(ogpos.X, ogpos.Y, ogpos.Z)
end
					...
					if not reset then
						...
						if croot then
							...
							if dist2 > 2 then
								task.wait(3)
								print("GOING")
								croot.CFrame = CFrame.new(ogpos.X, ogpos.Y, ogpos.Z)
								task.wait(1)
								print("ARRIVED")
								mouse1click()
								task.wait(0.5)
	...
	while true do
		...
		if current == "F1" and lastkey ~= "F1" then
			...
			if toggle then
				...
				notify("auto fish stopped", "info")
			end		
		elseif current == "F2" and lastkey ~= "F2" then
			toggle2 = not toggle2
			if toggle2 then
				notify("auto lumber on", "info")
				keypress(0x38)
				task.wait(0.2)
				keyrelease(0x38)
				task.spawn(function()
				...
				notify("auto lumber turned off", "info")
			end
		elseif current == "F3" and lastkey ~= "F3" then
			...
			if toggle3 then
				...
				notify("stopped looking for roka", "info")
			end			
		end
		lastkey = current
	end
...
notify("fishing bot running", "info")
task.wait(1)
notify("toggle autofish: f1. Have your rod at 0 and your bait, if you are using any, at 9.", "info")
notify("toggle autolumber: f2. Have your gun at 1 and your axe at 8.", "info")
notify("!!!NOTE:THERE MUST BE AN ITEM IN UR SECOND SLOT AND NO ITEM IN UR 3RD SLOT!!!", "warning")
